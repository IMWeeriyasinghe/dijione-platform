"""Mandatory MVP security requirement: Client A must never be able to reach
Client B's records via list, detail, manipulated route ID, or search/filter
endpoints. Now enforced from claims (client_id/client_ids embedded by
Platform Core), not a DB lookup — same repository-level scoping as before."""
...
def test_manipulated_route_id_is_blocked(api_client, db, two_tenant_world):
    abc_request, xyz_request = _create_requests(db, two_tenant_world)
    headers = two_tenant_world["xyz_headers"]
