"""Candidate Ownership Rule: one candidate can have applications against
multiple clients without ever being duplicated, and a client must only ever
see the client-safe view of their own application — never the candidate's
applications with other clients or internal notes."""
...
def test_candidate_can_apply_to_two_clients_as_one_master_record(db, two_tenant_world):
    ...
    xyz_request = request_service.create_request(
        client_id=two_tenant_world["xyz"].id, created_by=two_tenant_world["xyz_user_id"],
