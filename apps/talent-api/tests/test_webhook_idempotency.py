    ...
    events = list(
        db.execute(
            select(IntegrationEvent).where(
                IntegrationEvent.provider == "LEVER", IntegrationEvent.external_event_id == "evt-lever-001"
            )
        ).scalars()
    )
    assert len(events) == 1
...
def test_duplicate_hubspot_webhook_is_not_reprocessed(api_client, db):
    ...
    events = list(
        db.execute(
            select(IntegrationEvent).where(
                IntegrationEvent.provider == "HUBSPOT", IntegrationEvent.external_event_id == "evt-hubspot-001"
            )
        ).scalars()
    )
    assert len(events) == 1
