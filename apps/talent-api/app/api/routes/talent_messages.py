...
def list_messages(
    ...
    return [service.to_out(m) for m in messages]
...
def create_message(
    ...
    message = service.send_message(
        talent_request_id=request_id, sender_id=scope.user.id, sender_name=scope.user.full_name,
        sender_role=scope.role, body=payload.body,
    )
    db.commit()
    db.refresh(message)
    return service.to_out(message)
