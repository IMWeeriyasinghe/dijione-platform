...
issuer, audience, expiry, role claims — CLAUDE.md §12).

Local/demo target: DEV IDENTITY MODE (§13). ``DevAuthProvider`` issues and
...
class EntraAuthProvider(AuthProvider):
    ...
    available (CLAUDE.md §58). Wiring this in later must not require
    changes to route or service code, only to ``get_auth_provider``.
    """
