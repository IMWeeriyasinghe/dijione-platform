"""DijiOne Admin Center API (Phase 2 CR §47).

Every route depends on ``require_platform_admin`` (any admin — SUPER_ADMIN
or PLATFORM_ADMIN) at minimum; mutating routes further gate on specific
permissions via ``require_platform_permission`` so PLATFORM_ADMIN cannot
silently gain SUPER_ADMIN-only capabilities. Business-rule failures raised
by ``AdminService`` (not-found, last-SUPER_ADMIN lockout, admin-role
restriction) are translated to HTTP responses here — the service layer
...
@router.get("/clients")
def list_clients(_admin: User = Depends(require_platform_admin), db: Session = Depends(get_db)) -> list[dict]:
