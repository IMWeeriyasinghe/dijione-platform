...
def _module_roles_payload(user: User, repo: UserRepository) -> list[dict]:
    return [
        {"module_key": r.module_key, "role": r.role, "client_id": r.client_id}
        for r in repo.module_roles_for(user.id)
    ]


def _to_current_user_out(user: User, repo: UserRepository) -> CurrentUserOut:
...
def dev_login(payload: DevLoginRequest, db: Session = Depends(get_db)) -> TokenResponse:
    ...
    return TokenResponse(access_token=token, user=_to_current_user_out(user, repo))
...
def get_me(
    ...
    return _to_current_user_out(user, UserRepository(db))
