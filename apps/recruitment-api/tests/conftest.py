"""recruitment-api is an internal-only service: only DijiOne backends call
it, always with the shared internal token. Tests run against SQLite with
``INTEGRATIONS_MODE=mock`` — no Lever credential, no network.
"""
